...
# extends the seek behavior, and uses "chase the rabbit" to follow a path
class FollowPath(Behavior):
    def __init__(self, path, pathOffset, currentParam):
    ...
    def getSteering(self, character):
        ...
        self.currentParam = self.path.getParam(character.movement.position)
        targetParam = self.currentParam + self.pathOffset
        target = self.path.getPosition(targetParam)

        # create a target character to delegate to seek
        targetCharacter = Character(0, Movement(target, Vector2(0, 0), 0), SteeringOutput(), 0, 0, 0)

        #delegate to seek
        seek = Seek(targetCharacter)
        seek.getSteering(character)
